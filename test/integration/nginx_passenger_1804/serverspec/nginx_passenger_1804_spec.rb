...
describe 'opsworks_ruby::setup' do
  ...
  describe package('libnginx-mod-http-passenger') do
    it { should be_installed }
  end
...
describe 'opsworks_ruby::configure' do
  ...
  context 'appserver' do
    describe file('/etc/nginx/modules-enabled/50-mod-http-passenger.conf') do
      its(:content) { should include 'load_module /usr/lib/nginx/modules/ngx_http_passenger_module.so;' }
    end
    ...
    describe command('passenger-status') do
      ...
      its(:stdout) { should match('nginx/1.14.0')}
    end
